...
namespace UITool
{
    public class TextPresetWindow : DesignLibraryWindowBase
    {
        ...
        private void BuildDetail(VisualElement right)
        {
            ...
            aliasLabel.style.display = selected.generateCode ? DisplayStyle.Flex : DisplayStyle.None;

            var aliasField = new TextField();
            aliasField.value = selected.codeAlias ?? "";
            aliasField.style.height = 22;
            aliasField.style.marginTop = 2;
            aliasField.style.display = selected.generateCode ? DisplayStyle.Flex : DisplayStyle.None;
            aliasField.RegisterCallback<FocusOutEvent>(e =>
            {
                selected.codeAlias = aliasField.value;
                SaveAsset();
            });
